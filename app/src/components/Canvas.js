...
export default class Canvas extends React.Component {
  ...
  componentWillUnmount() {
    document.removeEventListener('dragover', e => {}, false);
  }
  ...
  onDropHandler(event) {
    ...
    if (node.properties.type.enum[0] === 'observable') {
      this.store.growlMessage =
        'Observables can only be dropped onto existing STIX objects.';
      this.store.showGrowl = true;
  ...
  drawEdge(persisted) {
    ...
    setTimeout(() => {
      ...
      let labelProps = LeaderLine.pathLabel(persisted.relationship_type, {
        color: '#484d59',
        outlineColor: '#484d59',
        fontWeight: '1px',
        letterSpacing: '2px',
